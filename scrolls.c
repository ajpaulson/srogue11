...
int read_scroll()
{
	...
	case S_KNOWALL:
		...
		when S_ARMOR : if (!curse)
		{
			if (cur_armor != NULL && o_off(cur_armor, ISPROT)) {
				...
				if (o_on(cur_armor, ISCURSED))
					cur_armor->o_ac =
					    armors[cur_armor->o_which].a_class;
				else
					cur_armor->o_ac--;
				resoflg(cur_armor, ISCURSED);
			}
		}
		when S_HOLD : if (!curse)
		{
			/*
			 * Hold monster scroll.  Stop all monsters within 3
			 * spaces
		...
		when S_CREATE : if (!bless)
		{
			if (makemons(mtlev[rnd(levcount)]->m_show))
				s_know[S_CREATE] = TRUE;
			else
				msg("You hear a faint cry of anguish in the "
				    "distance.");
		}
		when S_IDENT : if (!curse)
		{
			msg("This scroll is an identify scroll");
			s_know[S_IDENT] = TRUE;
			whatis(NULL);
		}
		when S_MAP : if (curse) break;
		s_know[S_MAP] = TRUE;
		...
		when S_GFIND : if (!curse)
		{
			...
			for (rp = rooms; rp < &rooms[MAXROOMS]; rp++) {
				...
				if (rp->r_goldval != 0 &&
					...
					mvwaddch(hw, rp->r_gold.y, rp->r_gold.x,
						 GOLD);
			}
			if (gtotal) {
				s_know[S_GFIND] = TRUE;
		...
		when S_ENCH : if (!curse)
		{
			if (cur_weapon == NULL ||
			    (cur_weapon != NULL &&
			     (o_on(cur_weapon, ISPROT) ||
		...
		when S_SCARE :
		     ...
		     * if it is dropped.  Thus reading it is a mistake and
		     * produces
		     * laughter at the poor rogue's boo boo.
		     */
		    msg("You hear maniacal laughter in the distance.");
		when S_REMOVE : if (!curse)
		{
			if (cur_armor != NULL && o_off(cur_armor, ISPROT))
				resoflg(cur_armor, ISCURSED);
		...
		when S_NOP : msg("This scroll seems to be blank.");
		when S_GENOCIDE : if (!curse)
		{
			msg("You have been granted the boon of genocide.");
			genocide();
			s_know[S_GENOCIDE] = TRUE;
		}
		when S_DCURSE : if (!bless)
		{
			...
			if ((ll = get_item("enchant", 0)) != NULL) {
				...
				case RING:
					...
					if (lb->o_ac > ac)
						lb->o_ac = ac;
					lb->o_ac -= howmuch;
					when STICK : lb->o_charges +=
					    howmuch + 10;
					when WEAPON : if (lb->o_dplus < 0)
						      lb->o_dplus = 0;
					if (lb->o_hplus < 0)
						lb->o_hplus = 0;
					lb->o_hplus += howmuch;
					lb->o_dplus += howmuch;
				otherwise:
					msg("You are injured as the scroll "
					    "flashes & bursts into flames !!!");
		...
		when S_MAKEIT : if (!curse)
		{
			msg("You have been endowed with the power of "
			    "creation.");
		...
		when S_LOCTRAP:
		{
			...
			if (ntraps > 0) {
				for (trp = &traps[0]; trp < &traps[ntraps];
				     trp++)
