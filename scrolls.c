  ...
  case S_KNOWALL:
    ...
    when S_GFIND : if (!curse)
    {
      ...
      for (rp = rooms; rp < &rooms[MAXROOMS]; rp++) {
        gtotal += rp->r_goldval;
        if (rp->r_goldval != 0 && mvinch(rp->r_gold.y, rp->r_gold.x) == GOLD)
      ...
      if ((ll = get_item("enchant", 0)) != NULL) {
        ...
        case RING:
          if (lb->o_ac < 0)
            lb->o_ac = 0;
          lb->o_ac += howmuch;
          when ARMOR : ac = armors[lb->o_which].a_class;
          if (lb->o_ac > ac)
            lb->o_ac = ac;
          lb->o_ac -= howmuch;
          when STICK : lb->o_charges += howmuch + 10;
          when WEAPON : if (lb->o_dplus < 0) lb->o_dplus = 0;
          if (lb->o_hplus < 0)
            lb->o_hplus = 0;
          lb->o_hplus += howmuch;
          lb->o_dplus += howmuch;
