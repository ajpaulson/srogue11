	...
	while (ntimes-- > 0) {
		...
		if (player.t_nocmd <= 0) {
			...
			if (running)
				...
				ch = readchar();
				if (mpos != 0 &&
				    !running) /* Erase message if its there */
					msg("");
			}
		} else
			ch = '.';
		if (player.t_nocmd > 0) {
			...
			case '!':
				...
				after = FALSE;
				when '#' : if (levtype ==
					       POSTLEV) /* buy something */
					   buy_it();
				after = FALSE;
				when '$' : if (levtype ==
					       POSTLEV) /* price something */
					   price_it();
				after = FALSE;
				when '%' : if (levtype ==
					       POSTLEV) /* sell something */
					   sell_it();
				after = FALSE;
			otherwise:
				after = FALSE;
...
call()
{
	...
	case RING:
		...
		elsewise = (ws_guess[wh] != NULL ? ws_guess[wh]
						 : ws_stuff[wh].ws_made);
	otherwise:
