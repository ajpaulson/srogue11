  ...
  while (ntimes-- > 0) {
    ...
    if (player.t_nocmd > 0) {
      ...
      if (isdigit(ch)) {
        ...
        countch = ch;
        /*
         * turn off count for commands which don't make
         * sense
         * to repeat
         */
        switch (ch) {
      ...
      case '!':
        ...
        when 's' : search();
        when 'z' : do_zap(FALSE);
        when 'p' : if (get_dir()) do_zap(TRUE);
        else after = FALSE;
        when 'v' : msg("Super Rogue version %s.", release);
        when 'D' : dip_it();
        when CTRL('L') : after = FALSE;
        restscr(cw);
        when CTRL('R') : after = FALSE;
        msg(huh);
        when 'a' : after = FALSE;
        dispmax();
        when '@' : if (author()) msg("Hero @ %d,%d : Stairs @ %d,%d", hero.y,
                                     hero.x, stairs.y, stairs.x);
        when 'S' : after = FALSE;
        if (save_game()) {
          wclear(cw);
          draw(cw);
        ...
        count = 0;
        after = FALSE;
        when '#' : if (levtype == POSTLEV) /* buy something */
                   buy_it();
        after = FALSE;
        when '$' : if (levtype == POSTLEV) /* price something */
                   price_it();
        after = FALSE;
        when '%' : if (levtype == POSTLEV) /* sell something */
                   sell_it();
        after = FALSE;
      otherwise:
        after = FALSE;
    ...
    if (pl_on(ISHELD)) {
      ...
      if (amulet) {
        ...
        if (level == 0)
          total_winner();
        new_level(NORMLEV);
        msg("You feel a wrenching sensation in your "
            "gut.");
        return 0;
      }
    }
...
int call()
{
  ...
  case RING:
    ...
    when POTION : guess = p_guess;
    elsewise = (p_guess[wh] != NULL ? p_guess[wh] : p_colors[wh]);
    when SCROLL : guess = s_guess;
    elsewise = (s_guess[wh] != NULL ? s_guess[wh] : s_names[wh]);
    when STICK : guess = ws_guess;
    elsewise = (ws_guess[wh] != NULL ? ws_guess[wh] : ws_stuff[wh].ws_made);
  otherwise:
