...
bool drop;
{
	...
	case SCROLL:
		...
		if (p_know[wh] || o_on(obj, ISPOST)) {
			...
			sprintf(prbuf, "%s%s %s %s%s", nm,
				vowelstr(p_colors[wh]), p_colors[wh], tn, pl);
		when FOOD : if (wh == 1)
		{
			if (obj->o_count == 1)
				q = vowelstr(fruit);
		...
		if (o_on(obj, ISKNOW | ISPOST)) {
			knowit = TRUE;
			sprintf(prbuf, "%s %s",
				num(armors[wh].a_class - obj->o_ac, 0), inm);
		} else
			sprintf(prbuf, "%s", inm);
		...
		when STICK:
		{
			...
			if (ws_know[wh] || o_on(obj, ISPOST)) {
				knowit = TRUE;
				sprintf(pb, "of %s%s", ws_magic[wh].mi_name,
					charge_str(obj));
				if (ws_know[wh]) {
					pb = &prbuf[strlen(prbuf)];
				...
				sprintf(prbuf, "A%s %s %s",
					vowelstr(rd->ws_made), rd->ws_made,
					rd->ws_type);
		}
		...
		else if (r_guess[wh]) sprintf(prbuf, "A %s called %s(%s)", tn,
					      r_guess[wh], r_stones[wh]);
		else sprintf(prbuf, "A%s %s %s", vowelstr(r_stones[wh]),
			     r_stones[wh], tn);
	otherwise:
...
void money()
{
	...
	if (rp != NULL && ce(hero, rp->r_gold)) {
		...
		for (item = mlist; item != NULL; item = next(item)) {
			tp = THINGPTR(item);
			if (rnd(100) < 70 && tp->t_room == rp &&
			    !iswearing(R_STEALTH) &&
			    ((tp->t_flags & (ISMEAN | ISGREED)) ||
			     rnd(1000) < 20))
				runto(&tp->t_pos, &hero);
		}
	} else
