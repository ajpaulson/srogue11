...
bool drop;
{
  ...
  case SCROLL:
    ...
    when POTION : sprintf(prbuf, "%s %s%s ", nm, tn, pl);
    pb = &prbuf[strlen(prbuf)];
    if (p_know[wh] || o_on(obj, ISPOST)) {
      ...
      sprintf(prbuf, "%s%s %s %s%s", nm, vowelstr(p_colors[wh]), p_colors[wh],
              tn, pl);
    when FOOD : if (wh == 1)
    {
      if (obj->o_count == 1)
    ...
    knowit = TRUE;
    when WEAPON : inm = w_magic[wh].mi_name;
    strcpy(prbuf, nm);
    if (obj->o_count == 1)
      q = vowelstr(inm);
    pb = &prbuf[strlen(prbuf)];
    ...
    strcat(prbuf, pl);
    when ARMOR : inm = a_magic[wh].mi_name;
    if (o_on(obj, ISKNOW | ISPOST)) {
      knowit = TRUE;
      sprintf(prbuf, "%s %s", num(armors[wh].a_class - obj->o_ac, 0), inm);
    } else
      sprintf(prbuf, "%s", inm);
    when AMULET : strcpy(prbuf, "The Amulet of Yendor");
    when STICK:
    {
      struct rod *rd;
      ...
      if (ws_know[wh] || o_on(obj, ISPOST)) {
        ...
        sprintf(prbuf, "A%s %s %s", vowelstr(rd->ws_made), rd->ws_made,
                rd->ws_type);
    }
    when RING : if (r_know[wh] || o_on(obj, ISPOST))
    {
      knowit = TRUE;
      sprintf(prbuf, "A%s %s of %s", ring_num(obj), tn, r_magic[wh].mi_name);
      if (r_know[wh]) {
        pb = &prbuf[strlen(prbuf)];
        ...
        sprintf(prbuf, "A %s called %s(%s)", tn, r_guess[wh], r_stones[wh]);
    else sprintf(prbuf, "A%s %s %s", vowelstr(r_stones[wh]), r_stones[wh], tn);
  otherwise:
  ...
  if (ch == POOL) {
    ...
    if (levtype == POSTLEV) {
      ...
      if (item == NULL) /* if item wasn't sold */
        msg("Thanks for your donation to the Fiend's "
            "flea market.");
    } else {
      attach(lvl_obj, ll);
 ...
 */
bool dropcheck(op) struct object *op;
{
  if (op == NULL)
    return TRUE;
  if (levtype == POSTLEV) {
  ...
  if (op != cur_armor && op != cur_weapon && op != cur_ring[LEFT] &&
      op != cur_ring[RIGHT])
    return TRUE;
  if (o_on(op, ISCURSED)) {
 ...
 *	Return a new thing
 */
struct linked_list *new_thing(treas, type, which) int type, which;
bool treas;
{
  ...
  case TYP_AMULET:
    ...
    when TYP_POTION : cur->o_type = POTION;
    cur->o_which = which;
    cur->o_count += extras();
    cur->o_vol = itemvol(cur);
    when TYP_SCROLL : cur->o_type = SCROLL;
    cur->o_which = which;
    cur->o_count += extras();
    cur->o_vol = itemvol(cur);
    when TYP_FOOD : no_food = 0;
    initfood(cur);
    when TYP_WEAPON : cur->o_which = which;
    ...
    when TYP_ARMOR : cur->o_which = which;
    initarmor(cur, which);
    if ((chance = rnd(100)) < 20) {
      ...
      cur->o_ac -= rnd(3) + 1;
    when TYP_RING : cur->o_which = which;
    init_ring(cur, FALSE);
    when TYP_STICK : default : cur->o_which = which;
    fix_stick(cur);
  }
  return item;
}
 ...
 */
void basic_init(cur) struct object *cur;
{
  cur->o_ac = 11;
  cur->o_count = 1;
  cur->o_launch = 0;
