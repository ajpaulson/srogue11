 ...
 */
bool fight(mp, weap, thrown) struct coord *mp;
struct object *weap;
bool thrown;
{
    ...
    if (pl_on(ISINVINC)) {
      ...
      if (off(*mp, ISCANC))
        ...
        case 'R':
          ...
          when 'Q' : if (!save(VS_POISON) && !iswearing(R_SUSAB))
          {
          ...
          when 'A' : if (!save(VS_POISON) && herostr() > MINABIL)
          {
          ...
          when 'W' : if (rnd(100) < 15 && !iswearing(R_SUSAB))
          {
            if (him->s_exp <= 0)
              death(mp->t_indx);
          ...
          when 'd' : if (rnd(100) < 50 && !(mp->t_flags & ISHUH))
          ...
          when 'g' : if (!save(VS_BREATH) && !iswearing(R_BREATH))
          {
            msg("You feel singed.");
            chg_hpt(-roll(1, 8), FALSE, mp->t_indx);
          }
          when 'h' : if (!save(VS_BREATH) && !iswearing(R_BREATH))
          {
            msg("You are seared.");
            chg_hpt(-roll(1, 4), FALSE, mp->t_indx);
          }
          when 'p' : if (!save(VS_POISON) && herostr() > MINABIL)
          {
            if (!iswearing(R_SUSTSTR) && !iswearing(R_SUSAB)) {
              msg("You are gnawed.");
              chg_abil(STR, -1, TRUE);
 ...
 */
bool roll_em(att, def, weap, hurl) struct stats *att, *def;
struct object *weap;
bool hurl;
{
  reg char *cp;
 ...
 */
void thunk(weap, mname) struct object *weap;
char *mname;
{
 ...
 */
void bounce(weap, mname) struct object *weap;
char *mname;
{
  if (weap->o_type == WEAPON)
 ...
 */
bool is_magic(obj) struct object *obj;
{
  switch (obj->o_type) {
  case ARMOR:
    ...
    if (rp != NULL) {
      if (rp->r_goldval != 0 || fallpos(&here, &rp->r_gold, FALSE)) {
        ...
        if (!save_throw(VS_MAGIC, tp))
          rp->r_goldval += GOLDCALC + GOLDCALC + GOLDCALC + GOLDCALC + GOLDCALC;
