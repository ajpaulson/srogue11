...
char type;
{
   ...
   */
  killexp =
      mp->m_stats.s_exp * (0.47 + (float)st->s_hpt / (8 * (float)st->s_lvl));
...
struct linked_list *wake_monster(y, x) int y, x;
{
  ...
  if (ch == 'U' && pl_off(ISBLIND)) {
    if ((rp != NULL && !rf_on(rp, ISDARK) && levtype != MAZELEV) ||
      ...
      if (off(*tp, ISFOUND) && !save(VS_PETRIFICATION) && !iswearing(R_SUSAB) &&
          pl_off(ISINVINC)) {
 ...
 */
void unhold(whichmon) char whichmon;
{
  switch (whichmon) {
  case 'F':
    fung_hit = 0;
 ...
 */
int midx(whichmon) char whichmon;
{
  if (isupper(whichmon))
