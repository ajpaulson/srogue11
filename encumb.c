 ...
 */
int itemweight(wh) struct object *wh;
{
	reg int weight;
 ...
 */
int itemvol(wh) struct object *wh;
{
	...
	case ARMOR:
		...
		when STICK
		    : if (strcmp(ws_stuff[wh->o_which].ws_type, "staff") == 0)
		      extra = V_WS_STAFF;
 ...
 */
int wghtchk(fromfuse) int fromfuse;
{
 ...
 */
int hitweight() { return (2 - foodlev); }
