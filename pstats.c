 ...
 */
void chg_hpt(howmany, alsomax, what) int howmany;
bool alsomax;
char what;
{
  nochange = FALSE;
 ...
 */
int chg_abil(what, amt, how) int amt, what, how;
{
  if (amt == 0)
    return 0;
 ...
 */
void add_haste(potion) bool potion;
{
  if (pl_on(ISHASTE)) {
 ...
 *	Heal the hero.
 */
void heal_self(factor, updmaxhp) int factor;
bool updmaxhp;
{
