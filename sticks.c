    ...
    case WS_SAPLIFE:
                ...
                for (j = hero.x - 3; j <= hero.x + 3; j++)
                {
                    ...
                    if (isalpha(mvwinch(mw, i, j)))
                    {
                        ...
                        case WS_ANNIH:
                            if (!curse)
                                killed(mitem, FALSE);
                            when WS_MREG : if (!bless) it->t_stats.s_hpt *= 2;
                            when WS_MDEG : if (!curse)
                            {
                                it->t_stats.s_hpt /= 2;
                                if (it->t_stats.s_hpt < 2)
                                    killed(mitem, FALSE);
                            }
                            when WS_PARZ : if (!curse)
                            {
                                it->t_flags |= ISPARA;
                                it->t_flags &= ~ISRUN;
                            }
        ...
        when WS_HASTE_M : case WS_CONFMON : case WS_SLOW_M : case WS_MOREMON:
        {
            ...
            if (isalpha(mvwinch(mw, y, x)))
            {
                ...
                else if (wh == WS_CONFMON && !curse)
                { /* confuse it */
                    tp->t_flags |= ISHUH;
                    if (pl_on(ISHELD) && tp->t_type == 'd')
                        player.t_flags &= ~ISHELD;
                }
                else if (wh == WS_SLOW_M && !curse)
                { /* slow it */
                    if (on(*tp, ISHASTE))
                        tp->t_flags &= ~ISHASTE;
                    else
