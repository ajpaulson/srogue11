 ...
 */
void fix_stick(cur) struct object *cur;
{
	struct rod *rd;
	...
	case WS_HIT:
		...
		when WS_LIGHT : cur->o_charges += 7 + rnd(9);
	}
}
...
int do_zap(gotdir) bool gotdir;
{
	...
	case WS_SAPLIFE:
		...
		when WS_PARZ : case WS_MREG : case WS_MDEG : case WS_ANNIH:
		{
		...
		when WS_POLYM : case WS_TELAWAY : case WS_TELTO
						  : case WS_CANCEL
						    : case WS_MINVIS:
		{
			...
			if (isalpha(monster = mvwinch(mw, y, x))) {
				...
				if (wh == WS_POLYM && !curse) {
					...
					item =
					    new_monster(monster, &delta, FALSE);
					if (!(tp->t_flags & ISRUN))
						runto(&delta, &hero);
					if (isalpha(mvwinch(cw, y, x)))
						mvwaddch(
						    cw, y, x,
						    monsters[monster].m_show);
					tp->t_oldch = oldch;
					ws_know[WS_POLYM] |=
					    (monster != omonst);
					...
					mvwaddch(mw, tp->t_pos.y, tp->t_pos.x,
						 monster);
					tp->t_oldch = mvwinch(cw, tp->t_pos.y,
							      tp->t_pos.x);
				}
			}
		}
		...
		when WS_NOP : msg("Your %s flickers momentarily and then fades",
				  ws_stuff[wh].ws_type);
		when WS_HIT:
		{
			char ch;
		...
		when WS_HASTE_M : case WS_CONFMON : case WS_SLOW_M
						    : case WS_MOREMON:
		{
			reg int m1, m2;
			struct coord mp;
 ...
 */
char *charge_str(obj) struct object *obj;
{
	static char buf[20];
