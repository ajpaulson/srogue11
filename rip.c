...
char *reason[] = {"Killed", "Chickened out", "A Total Winner"};
int oldpurse;
...
int amount, aflag;
{
  ...
  for (i = 0; i < 10; i++) {
    ...
    sscanf(scoreline, " %d %d %d %d %u %d %ld %lx \n", &top_ten[i].sc_score,
           &top_ten[i].sc_flags, &top_ten[i].sc_level, &top_ten[i].sc_uid, &mon,
           &top_ten[i].sc_explvl, &top_ten[i].sc_exppts, &top_ten[i].sc_date);
  ...
  for (i = 0; i < 10; i++) {
    ...
    sprintf(scoreline, " %d %d %d %d %u %d %ld %lx \n", top_ten[i].sc_score,
            top_ten[i].sc_flags, top_ten[i].sc_level, top_ten[i].sc_uid,
            top_ten[i].sc_monster, top_ten[i].sc_explvl, top_ten[i].sc_exppts,
            top_ten[i].sc_date);
...
bool showtop(showname) int showname;
{
  ...
  for (scp = top_ten; scp <= &top_ten[9]; scp++) {
    if (scp->sc_score > 0) {
      printf("%d\t%d\t%s: %s\t\t--> %s on level %d", scp - top_ten + 1,
...
void total_winner()
{
  ...
  addstr("     Congratulations, you have made it to the light of day!    "
         "\n");
  addstr("\nYou have joined the elite ranks of those who have escaped "
         "the\n");
  addstr("Dungeons of Doom alive.  You journey home and sell all your "
         "loot at\n");
 ...
 */
char *killname(monst) unsigned char monst;
{
  if (monst < MAXMONS + 1)
