...
int do_move(dy, dx) int dy, dx;
{
   ...
   */
  if ((rnd(100) < 80 && pl_on(ISHUH)) || (iswearing(R_DELUS) && rnd(100) < 25))
    nh = *rndmove(&player);
  else {
    nh.y = hero.y + dy;
    nh.x = hero.x + dx;
 ...
 */
int be_trapped(tc, th) struct thing *th;
struct coord *tc;
{
  reg struct trap *trp;
  reg int ch, ishero;
  ...
  case POST:
    ...
    else
    {
      ...
      if (seeit)
        msg("%s fall%s asleep in a strange white mist.", stuckee,
            (ishero ? "" : "s"));
    }
    when ARROWTRAP:
    {
      ...
      if (swing(3 + (level / 4), resist, 1)) {
        if (seeit)
          msg("%sAn arrow shot %s.", (ishero ? "Oh no! " : ""), stuckee);
        if (ishero)
