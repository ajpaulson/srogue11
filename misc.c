      ...
      for (y = hero.y - 1; y <= ey; y += 1) {
        ...
        if (door_stop && !firstmove && running) {
          ...
          case 'h':
            ...
            when 'y' : if ((x + y) - (hero.x + hero.y) >= 1) continue;
            when 'u' : if ((y - x) - (hero.y - hero.x) >= 1) continue;
            when 'n' : if ((x + y) - (hero.x + hero.y) <= -1) continue;
            when 'b' : if ((y - x) - (hero.y - hero.x) <= -1) continue;
          }
          switch (ch) {
          case DOOR:
 ...
 */
bool is_current(obj) struct object *obj;
{
  if (obj == NULL)
    return FALSE;
 ...
 */
void initfood(what) struct object *what;
{
  what->o_type = FOOD;
