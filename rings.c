 ...
 */
void toss_ring(what) struct object *what;
{
  bool okring;
  ...
  if (okring) {
    ...
    case R_SPEED:
      ...
      when R_GIANT : him->s_ef = him->s_re;
      ringabil();
      when R_ADDSTR : chg_abil(STR, -what->o_ac, FALSE);
      when R_KNOW : chg_abil(WIS, -what->o_ac, FALSE);
      when R_DEX : chg_abil(DEX, -what->o_ac, FALSE);
      when R_CONST : chg_abil(CON, -what->o_ac, FALSE);
      when R_SEEINVIS : player.t_flags &= ~CANSEE;
      extinguish(unsee);
      light(&hero);
      mvwaddch(cw, hero.y, hero.x, PLAYER);
...
int ring_eat()
{
  ...
  for (i = LEFT; i <= RIGHT; i += 1) {
    ...
    if (lb != NULL) {
      ...
      case R_GIANT:
        howmuch += 2;
        when R_SPEED : case R_SUSTSTR : case R_SUSAB : howmuch += 1;
        when R_SEARCH : howmuch += (rnd(100) < 33);
        when R_DIGEST : switch (lb->o_ac)
        {
        case -3:
          if (rnd(100) < 25)
            howmuch += 3;
          when - 2 : if (rnd(100) < 50) howmuch += 2;
          when - 1 : howmuch += 1;
          when 0 : howmuch -= (rnd(100) < 50);
          when 3 : if (rnd(100) < 25) howmuch -= 3;
          when 2 : if (rnd(100) < 50) howmuch -= 2;
        default:
          howmuch -= 1;
        }
 ...
 *	Print ring bonuses
 */
char *ring_num(what) struct object *what;
{
 ...
 */
bool magring(what) struct object *what;
{
  switch (what->o_which) {
  case R_SPEED:
  case R_ADDSTR:
  ...
  for (i = LEFT; i <= RIGHT; i++) {
    ...
    if (rptr != NULL) {
      ...
      case R_ADDSTR:
        ...
        when R_DEX : chg_abil(DEX, rptr->o_ac, FROMRING);
        when R_KNOW : chg_abil(WIS, rptr->o_ac, FROMRING);
        when R_CONST : chg_abil(CON, rptr->o_ac, FROMRING);
      }
    }
  }
}
...
bool fromwiz; /* TRUE when from wizards */
{
  ...
  case R_CONST:
    ...
    when R_SPEED : what->o_ac = rnd(4) + 1;
    when R_AGGR : case R_DELUS : case R_HEAVY : case R_BLIND
                                                : case R_SLOW
                                                  : case R_SAPEM
                                                    : case R_TELEPORT
                                                      : what->o_ac = 0;
    setoflg(what, ISCURSED);
    when R_GIANT : what->o_ac = 25; /* lots !! of STR */
  otherwise:
    what->o_ac = 1;
  }
  what->o_type = RING;
 ...
 */
bool isring(hand, ring) int hand, ring;
{
